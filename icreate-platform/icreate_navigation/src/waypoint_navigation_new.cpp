...
void setGoalSucceedEvent(icreate::SingleNavigation &single_navigation) {
    ROS_INFO("SUCCEEDED %s",single_navigation.robot.current_state.c_str());
    single_navigation.doneRobotGoal();
}
...
int main(int argc, char** argv) {
    ...
    // single_navigation.robot.setCurrentPosition("map", "base_footprint", "Building 4", "Floor 20");
    // waitMoveBaseServer(ac);

    single_navigation.showNavigationMenu(base_frame_id, robot_frame_id);
    ...
    while(ros::ok()) {
        ...
        if(isDoneGoal) {
			...
			if(doneGoalNumber == 1)
			{
				setGoalSucceedEvent(single_navigation);
                isNextStep = true;
			}
            else {
                single_navigation.showNavigationMenu(base_frame_id, robot_frame_id);
        ...
        if(isNextStep) {
            ...
            // transportation.aa();
            single_navigation.setNextStepMode();
        }

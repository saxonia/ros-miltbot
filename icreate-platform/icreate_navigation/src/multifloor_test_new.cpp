...
int main(int argc, char** argv) {
    ...
    new_point.target_pose.pose.position.x = 1;
    new_point.target_pose.pose.orientation.w = 1;
    icreate::MoveBaseGoalData data("Going To Lift", new_point, multi_navigation.navigations_[multi_navigation.nav_idx].building, multi_navigation.navigations_[multi_navigation.nav_idx].building_floor_lift);
    robot.target_queue.insert(robot.target_queue.begin(),data);
    multi_navigation.nav_idx = 1;
    multi_navigation.navigation_case = 1;
    multi_navigation.lift_navigation_step = 2;
    multi_navigation.runLiftNavigation(robot);
    multi_navigation.navigations_[multi_navigation.nav_idx].requestToSetNewGoal = true;
    ...
    while(ros::ok()) {
        ...
        if(multi_navigation.navigations_[multi_navigation.nav_idx].requestToSetNewGoal) {
            ...
            if(multi_navigation.lift_navigation_step > 1 && multi_navigation.lift_navigation_step < 6) {
                ...
                // multi_navigation.navigations_[multi_navigation.nav_idx].setRobotGoal("base_footprint");
                multi_navigation.navigations_[multi_navigation.nav_idx].setRobotGoal(base_frame_id);
                // sing.setRobotGoal(robot_frame_id);
            }
            else {
                ROS_INFO("Loop Set New Goal 2");
                // multi_navigation.navigations_[multi_navigation.nav_idx].setRobotGoal("base_link");
                multi_navigation.navigations_[multi_navigation.nav_idx].setRobotGoal(base_frame_id);
            }
            

            ROS_WARN("X_PO : %lf",multi_navigation.navigations_[multi_navigation.nav_idx].getRobotGoal().target_pose.pose.position.x);
